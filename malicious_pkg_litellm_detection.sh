...
# Jamf Extension Attribute: litellm version detection
# Detects installations of litellm versions 1.82.7 or 1.82.8 statically.
...
# Function to check for litellm metadata in a site-packages dir
check_litellm() {
    local base_dir="$1"
    for version in "1.82.7" "1.82.8"; do
        if [[ -d "${base_dir}/litellm-${version}.dist-info" ]]; then
            add_to_unique "${base_dir}/litellm-${version}.dist-info"
        fi
    done
}
